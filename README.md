...
| 分类 | 能力 |
|---|---|
| **入站** | 公屏消息、私聊消息、房间成员上线 / 下线 / 重连 / 换房（换房会带上目标房间名） |
| **出站** | 文本、`@用户`、引用回复、图片、表情包（经图床转链） |
| **消息格式** | 自动转换 IIROSE 原生语法：`[*用户名*]` 提及、`[@uid@]` 按 id 提及、`(_hr) 发送者_时间戳 (hr_)` 引用、`[url#e]` 图片、点歌 / 点播卡片 |
| **房间名** | 从 `%` 大包解析房间目录，用作群聊会话名（后台聊天列表显示的名字）与成员事件文案 |
| **图片 / 表情** | 对接自建图床：base64 → 上传换 URL → 以 `[url#e]` 发送；按内容指纹复用链接，链接失效自动重传 |
| **名单过滤** | 黑白名单（按 IIROSE 唯一标识 UID 或用户名），三个名单各自独立开关 |
| **改房间实时生效** | 配置里换房间号后自动发切房指令、下线重连进新房间 |
...
> WebUI 配置面板的**字段名和说明都是中文**，配置节按 `① 插件设置` → `⑥ 图床` 编号排列，方便对照。
> 下面表格里列出的是 `config.toml` 里的原始键名，两种方式改的是同一份配置。
...
三个名单**各自独立开关**，互不影响；`enabled` 是总开关（默认关闭，关闭时三项都不生效）。

| 字段 | 默认 | 说明 |
|---|---|---|
| `enabled` | `false` | **总开关**：关闭时下面三项名单全部不生效 |
| `user_list_enabled` | `false` | 用户名单独立开关 |
| `user_list_type` | `"whitelist"` | `whitelist` 只回名单内的人 / `blacklist` 不回名单内的人 |
| `user_list` | `[]` | 用户名单，填 UID；也支持直接填用户名（按昵称兜底匹配） |
| `ban_user_enabled` | `false` | 永久屏蔽名单独立开关 |
| `ban_user_list` | `[]` | 永久屏蔽：这些人无论名单模式如何都不回复，优先级最高 |
| `room_list_enabled` | `false` | 房间名单独立开关 |
| `room_list_type` | `"blacklist"` | 房间名单模式 |
| `room_list` | `[]` | 房间 ID 名单 |
...
[chat_list]
enabled = true            # 总开关
user_list_enabled = true  # 只启用用户白名单
user_list_type = "whitelist"
user_list = ["16a7c38409e902", "某个用户名"]   # UID 或用户名
ban_user_enabled = false
room_list_enabled = false
```

判定顺序：**永久屏蔽 → 房间名单 → 用户名单**（语义对齐官方 NapCat 适配器的 `filters.py`）。

两点行为说明：

- **名单为空 = 该名单不生效**（不拦任何人），只打一条 warning 提醒你去填。这样不会出现「开了开关但还没填人，机器人突然装死」。
- 被拦下的消息**不会进入麦麦的上下文**，也不计入频率统计；关掉开关后立即恢复。
- 启动日志会打印生效状态，例如 `总开关=开 | 永久屏蔽=关 | 用户=白名单(2 条) | 房间=关`。
...
IIROSE 登录成功（服务端首个报文类型=init），当前房间=16a7c38409e902
...
如果日志出现 `IIROSE 切房被拒绝（错误码 5：未提供房间密码）`，说明目标房间有密码，填上 `room_password` 再改一次即可。
...
| `旧内容 (_hr) 发送者_时间戳秒 (hr_) 新内容` | 引用回复 | `[引用 发送者：旧内容] 新内容` | 还原成原生引用语法（时间戳取自本地近期消息缓存） |
...
| 现象 | 排查方向 |
|---|---|
| **麦麦突然不回复任何人了** | 先看启动日志里那行 `聊天名单：...`。空名单现在不会拦人，但仍建议核对 UID（个人资料页的「唯一标识」）是否填对 |
| **日志有入站，但麦麦不回复** | ① 看有没有 `IIROSE 名单过滤：忽略 xxx 的消息`；② 检查麦麦自己的聊天名单 / 回复频率 / 是否被 `@`；③ 看有没有 `IIROSE 已发送出站消息` |
...
| **改了房间号没生效** | 看有没有 `IIROSE 房间配置变更` 那行；没有说明配置没热更新（确认是在插件配置里改的）。出现 `切房被拒绝（错误码 5）` 就是缺房间密码 |
| **频繁掉线** | 先看是不是有 `IIROSE 连接断开(...)`。默认已开启 30 秒保活 + 120 秒停滞检测；网络差可调大 `stall_timeout_seconds` |
| **停用插件后机器人还在线** | 正常停用会打印 `IIROSE 连接已关闭（已下线）`。IIROSE 没有下线报文，靠的就是断开 WebSocket；若日志里没有这一行，说明 `on_unload` 没被触发（多为 Runner 直接被杀，此时系统会自动断开连接） |
| **成员事件和聊天像是两个会话** | 说明配置的 `room_id` 与服务端下发的房间号不一致，日志里有 `IIROSE 成员事件里的房间=... 按配置房间 ... 上报` |
| **机器人自问自答** | `[account] uid` 没填，填上即可 |
| **机器人不认自己被 @** | 检查 `[account] username` / `uid` 是否填成了界面上的带括号形式（`[*03酱*]` / `[@uid@]`）——这两种写法本插件会自动剥离，但用户名必须和 IIROSE 登录名完全一致 |
| **群聊在后台只显示一串房间号** | 房间名来自进房时的 `%` 大包，日志里会打 `解析出 N 个房间名`。若为 0，把那条 `IIROSE 服务端数据包` 日志发出来对齐格式 |
...
├── test_outbound_decode.py      # 回归测试（88 项，无需 pytest）
└── docs/
...
测试用桩替换了 `maibot_sdk` 与网络层，**不联网**，一两秒跑完，覆盖：

- 出站组件解码（文本 / @ / 引用 / 图片 / 点歌卡片）、目标解析
- 入站报文解析（公屏、私聊、成员事件、引用链、点歌卡片）
- 图床上传、复用、链接校验与重传（含模拟插件重启）
- 黑白名单三个独立开关、空名单行为、拦截时机
- 改房间号的切房流程（`m` 指令 + `lr` 重连）
- 连接保活、停滞检测、退避不溢出与「永不放弃重连」
